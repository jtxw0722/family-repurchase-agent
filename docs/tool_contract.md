...
* 查询历史最低价、历史中位价、历史平均价或价格基准线时，必须优先使用 `get_price_baseline`，不应误用 `generate_report`。
...
比较当前商品单位价格与本地历史购买记录，返回价格判断结果。

如果用户只是查询历史最低价、历史中位价、历史平均价或价格基准线，而没有提供当前价格，应使用 `get_price_baseline`，不要调用 `compare_price`。
...
| `price`       | number |  是 | 当前总价或实付金额                    |
| `quantity`    | number |  是 | 当前数量                         |
| `unit`        | string |  是 | 计量单位，例如 `kg`、`L`、`片`、`包`、`瓶` |
...
* `quantity` 必须大于 0；
* `unit` 必须是非空字符串；
* 当前版本要求调用方显式提供规格和单位，不由 LLM 直接猜测价格口径。
...
| `current`        | object | 当前价格计算结果  |
| `baseline`       | object | 历史价格基准线   |
| `decision`       | object | 价格判断结论    |
...
## 4. Tool: `get_price_baseline`

### 4.1 作用

查询某个复购品的本地历史价格基准线，包括历史最低价、历史中位价、历史平均价、样本数量、日期范围、evidence 和 warnings。
...
该工具不需要当前价格。判断“当前价格是否值得买”时应使用 `compare_price`。
...
```text
POST /api/tools/get-price-baseline
```
...
| 字段            | 类型     | 必填 | 说明                                       |
| ------------- | ------ | -: | ---------------------------------------- |
| `productName` | string |  是 | 原始商品名称，例如 `猫砂`、`纸巾`、`洗衣液`                |
| `unit`        | string |  否 | 可选统计单位，例如 `kg`、`抽`、`L`；为空时由商品归一化规则推断标准单位 |
...
* `productName` 必须是非空字符串；
* `unit` 可选；
* 当 `unit` 为空时，后端会根据商品归一化规则推断标准单位，例如：

    * `猫砂` -> `kg`
    * `纸巾` -> `抽`
    * `洗衣液` -> `L`
* 如果商品无法推断标准单位，后端可能要求调用方显式提供 `unit`。
...
`get_price_baseline` 的 `evidence` 结构与 `compare_price` 保持一致。
...
`warnings` 是字符串数组，用于提示历史基准线的可靠性风险。
...
| 平均值明显高于中位数 | `历史平均值明显高于中位数，可能存在异常值，建议优先参考历史中位数和历史最低价。` |
| 单位不一致样本被排除 | `存在 1 条历史记录单位不是 kg，已排除，不参与本次价格判断。`        |
| 无历史记录      | `历史记录不足，无法形成可靠价格基准线。`                     |

Agent Host 在回答用户时必须保留 warnings 的含义，不应只输出一个最低价数字。
...
| ---------------- | ------------------------------------------------------ | --------------------- |
| `productName` 为空 | `productName must be a non-empty string`               | `invalid_input`       |
| `unit` 提供但为空字符串  | `unit must be a non-empty string when provided`        | `invalid_unit`        |
| 商品无法推断标准单位       | 由后端应用服务返回                                              | `unit_required`       |
| 后端不可用            | `Failed to connect to Family Repurchase Agent backend` | `backend_unavailable` |
...
- `get_price_baseline` 当前返回历史基准线和代表性 evidence，暂不返回完整历史明细列表。