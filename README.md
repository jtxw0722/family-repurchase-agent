...
```powershell
curl -X POST "http://localhost:8080/api/tools/parse-order-image" `
  -H "Content-Type: application/json" `
  -d "{\"imagePath\":\"data/inbox/screenshots/order-001.png\",\"owner\":\"jtxw\",\"platform\":\"pdd\",\"parseMode\":\"order_screenshot\",\"dryRun\":true}"
```

`parse_order_image` 只返回结构化候选样本，不直接写入 `purchase_records`。用户确认后，应调用现有 `record_purchase`，或后续规划中的 `record_sample` 正式入库。
...
```powershell
python scripts/local-ocr.py --image data/inbox/screenshots/order-001.png
```

成功时 stdout 只输出识别文本，每个文本块一行。可以通过最小配置启用 Java 本地 OCR 适配器：
...
```text
python scripts/local-ocr.py --image {imagePath}
```
...
```powershell
java -jar target/family-repurchase-agent.jar `
  --family-agent.parse-order-image.local-ocr.enabled=true
```

本地环境使用其他 OCR 命令时，可以覆盖完整配置：
...
本地 OCR 命令需要把识别文本输出到 stdout。项目会读取 stdout 作为 OCR 原文，再交给 `OrderImageTextParser` 解析为候选样本；stderr 会作为 warning 或错误信息返回。

启动后可调用接口验证完整链路：

```powershell
curl.exe -X POST "http://127.0.0.1:8080/api/tools/parse-order-image" `
  -H "Content-Type: application/json" `
  -d "{\"imagePath\":\"data/inbox/screenshots/order-001.png\",\"owner\":\"jtxw\",\"platform\":\"pdd\",\"parseMode\":\"order_screenshot\",\"dryRun\":true}"
```

预期响应中 `rawText` 包含 OCR 原文、`candidateCount` 至少为 1，并包含只读边界 warning。OCR 只在本机运行，不会自动上传图片；识别效果取决于 RapidOCR、截图质量和页面布局。`parse_order_image` 仍只返回候选样本，不写入 `purchase_records`。
